...
def parse_args():
    ...
    parser.add_argument('--learning_rate', default=0.002, type=float, help='Initial learning rate [default: 0.002]')
...
def main(args):
    ...
    TRAIN_DATASET = PartNormalDataset(root = root, npoints=args.npoint, split='trainval', normal_channel=args.normal,class_choice=["Bag","Cap","Earphone","Rocket"])
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batch_size,shuffle=True, num_workers=4,drop_last=True)
    TEST_DATASET = PartNormalDataset(root = root, npoints=args.npoint, split='test', normal_channel=args.normal,class_choice=["Bag","Cap","Earphone","Rocket"])
    ...
    channel=6 if args.normal else 3
...
#  Train one epoch
        ...
        with torch.no_grad():
            total_loss1, total_loss2, i = 0, 0, 0
            for batch_id, (points1, points2, cls) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                i+=1
                points1, points2 = points1.cuda(), points2.cuda()
            ...
            log_string('EPOCH %d test: loss1 is: %.5f   loss2 is: %.5f ' % (epoch+1, total_loss1 / i, total_loss2 / i))
